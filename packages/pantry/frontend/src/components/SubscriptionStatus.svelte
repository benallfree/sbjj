<script lang="ts">
  ...
  const isUserFounder = $userSubscriptionType === SubscriptionType.Founder
...
<div class=" flex flex-col gap-4 mb-4">
  ...
  >
    <div class="card-body backdrop-blur-md text-white">
      ...
      {#if [SubscriptionType.Founder, SubscriptionType.FounderAnnual, SubscriptionType.Flounder].includes($userSubscriptionType)}
        <p>
          What an absolute chad you are. Thank you for supporting PocketHost
          with a {PLAN_NAMES[$userSubscriptionType]} membership!
        </p>
      {/if}
    </div>
  </div>
